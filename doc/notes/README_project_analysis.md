...
│               └── rvl_codec.h
├── launch
│   ├── adi_3dtof_adtf31xx_compressed_host_launch.py
│   ├── adi_3dtof_adtf31xx_launch.py
│   ├── adi_3dtof_adtf31xx_read_rosbag_launch.py
│   └── adi_3dtof_adtf31xx_rqt_launch.py
├── LICENSE
├── NOTICE
├── package.xml
├── README.md
...
└── src
    ├── adi_3dtof_adtf31xx_compressed_image_subscriber.cpp
...
| File | Description |
|------|-------------|
| `CMakeLists.txt` | ROS 2 CMake build configuration — defines build targets, dependencies, and install rules |
| `package.xml` | ROS 2 package manifest — declares package name, version, dependencies, and maintainer info |
| `LICENSE` | BSD 3-Clause license text |
| `NOTICE` | Third-party legal notices |
| `README.md` | Original project documentation |
| `README_project_analysis.md` | This project analysis document |

### config/

| File | Description |
|------|-------------|
| `config_adsd3500_adsd3100.json` | ToF SDK configuration for the ADSD3500 controller with ADSD3100 imager — sets depth compute parameters (thresholds, JBLF filter, AB, ISP settings) |
| `config_adsd3500_adsd3030.json` | ToF SDK configuration for the ADSD3500 controller with ADSD3030 imager |

### doc/

| File | Description |
|------|-------------|
| `conf.py` | Sphinx documentation configuration (project metadata, extensions, HTML theme) |
| `Makefile` | Linux Sphinx build script (`make html`, `make clean`, etc.) |
| `make.bat` | Windows Sphinx build script |
| `requirements.txt` | Python packages required to build Sphinx documentation |
| `images/architecture_diagram.png` | System architecture overview diagram |
| `images/connection_diagram.png` | Hardware connection diagram (sensor to host PC) |
| `images/depth_image.png` | Sample depth image output |
| `images/ir_image.png` | Sample AB (active brightness / IR) image output |
| `images/dynamic_reconfigure.png` | Screenshot of the rqt dynamic reconfigure GUI |

### include/

| File | Description |
|------|-------------|
| `adi_3dtof_adtf31xx.h` | Main ROS 2 node class header — declares `ADI3DToFADTF31xx` (extends `rclcpp::Node`) with publishers, threads, queues, and all node parameters |
| `adi_3dtof_adtf31xx_frame_info.h` | Frame data container class — holds raw depth, AB, XYZ, confidence buffers plus RVL-compressed variants captured from the sensor |
| `adi_3dtof_adtf31xx_output_info.h` | Output data container class — holds processed frame data ready for publishing to ROS topics |
| `adi_camera.h` | Structs for camera calibration — `CameraIntrinsics` (camera matrix, distortion coeffs) and `CameraExtrinsics` (rotation + translation) |
| `image_proc_utils.h` | Image processing utilities — generates a range-to-3D LUT using camera intrinsics for point cloud projection |
| `input_sensor.h` | Abstract base class `IInputSensor` — defines the interface for all input sources (open, configure, read frame, get intrinsics/extrinsics, close) |
| `input_sensor_adtf31xx.h` | Concrete sensor class `InputSensorADTF31XX` — implements `IInputSensor` using the ADI ToF SDK (`libaditof`) for live/network sensor access |
| `input_sensor_factory.h` | Factory class `InputSensorFactory` — instantiates the correct `IInputSensor` implementation based on `arg_input_sensor_mode` (0=sensor, 2=file-io, 3=network) |
| `input_sensor_file_rosbagbin.h` | File-IO sensor class `InputSensorFileRosbagBin` — implements `IInputSensor` by reading pre-recorded `.bin` files with default hardcoded camera intrinsics |
| `module_profile.h` | Compile-time function profiling macros — `PROFILE_FUNCTION_START/END` are no-ops unless `ENABLE_FUNCTION_PROFILING` is defined |
| `ros-perception/.../rvl_codec.h` | `RvlCodec` class header — RVL (Run-Length Variable-Length) lossless codec for compressing 16-bit depth images |
| `ros-perception/.../compression_common.h` | Shared compression constants and enums from `ros-perception/image_transport_plugins` (BSD license, Willow Garage) |

### launch/

| File | Description |
|------|-------------|
| `adi_3dtof_adtf31xx_launch.py` | Main launch file — starts the primary sensor node with all configurable parameters (sensor mode, file path, thresholds, camera mode, compression, publish toggles) |
| `adi_3dtof_adtf31xx_compressed_host_launch.py` | Launches the `adi_3dtof_adtf31xx_compressed_image_subscriber_node` on the host PC to receive and decompress RVL-compressed depth/AB images |
| `adi_3dtof_adtf31xx_read_rosbag_launch.py` | Launches the `read_rosbags` node to convert recorded rosbag files into `.bin` format for use in File-IO mode |
| `adi_3dtof_adtf31xx_rqt_launch.py` | Launches the `rqt_gui` with the pre-configured perspective for live parameter tuning via dynamic reconfigure |

### rmw_config/

| File | Description |
|------|-------------|
| `rmw_settings.xml` | FastDDS XML profile — increases `max_unicast_locators` to 256 to prevent DDS discovery failures on machines with many network interfaces |
| `setup_rmw_settings.sh` | Shell script — sets the `FASTRTPS_DEFAULT_PROFILES_FILE` env variable and appends it to `~/.bashrc`, then restarts the ROS 2 daemon |

### rqt_config/

| File | Description |
|------|-------------|
| `adi_3dtof_adtf31xx_rqt.perspective` | Saved rqt perspective layout — configures the rqt window with the dynamic reconfigure plugin pre-loaded for this node |

### rviz/

| File | Description |
|------|-------------|
| `adi_3dtof_adtf31xx.rviz` | RViz 2 configuration for visualizing depth image, AB image, confidence image, and point cloud together |
| `adi_3dtof_adtf31xx_raw.rviz` | RViz 2 configuration for displaying raw (unprocessed) image topics from the sensor |

### src/

| File | Description |
|------|-------------|
| `adi_3dtof_adtf31xx.cpp` | Main node implementation — initializes the node, declares all ROS parameters, creates publishers/subscribers, sets up input sensor via the factory, and spawns input/output threads |
| `adi_3dtof_adtf31xx_input_thread.cpp` | Input thread logic — continuously reads frames from the sensor or file, places them into the shared input queue for the output thread to process |
| `adi_3dtof_adtf31xx_output_thread.cpp` | Output thread logic — dequeues frames, applies optional RVL compression, and publishes depth, AB, confidence images, camera info, and point cloud to ROS topics |
| `adi_3dtof_adtf31xx_compressed_image_subscriber.cpp` | Standalone host subscriber node — subscribes to compressed depth/AB topics, decompresses them using RVL, and re-publishes as standard 16-bit images |
| `input_sensor_adtf31xx.cpp` | ADI ToF SDK sensor implementation — opens/configures the sensor over USB or network, reads frames (depth, AB, confidence, XYZ), and retrieves calibration data |
| `input_sensor_file_rosbagbin.cpp` | File-IO sensor implementation — opens and reads `.bin` files frame-by-frame, parsing the binary header for camera info and unpacking per-frame depth/AB data |
